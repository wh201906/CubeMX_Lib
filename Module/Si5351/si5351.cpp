...
Si5351::Si5351(uint8_t i2c_addr) : i2c_bus_addr(i2c_addr)
{
  ...
  SoftI2C_SetPort(&(this->port), GPIOE, 2, GPIOE, 3);
}
...
{
  SoftI2C_Init(&(this->port), 100000, SI2C_ADDR_7b);
...
{
  return (SoftI2C_Write(&(this->port), i2c_bus_addr, addr, data, bytes)?0:1);
}
...
{
  uint8_t reg_val = 0;
  SoftI2C_Read(&(this->port), i2c_bus_addr, addr, &reg_val, 1);
  return reg_val;
}
