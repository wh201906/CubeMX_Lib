...
void SoftI2C2_Init(uint32_t speed)
{
  ...
  SOFTI2C2_SCL(1);
  SOFTI2C2_SDA(1);
  GPIO_InitStruct.Pin = SOFTI2C2_SCL_PIN;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_OD;
  GPIO_InitStruct.Pull = GPIO_PULLUP;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
  HAL_GPIO_Init(SOFTI2C2_SCL_GPIO, &GPIO_InitStruct);

  GPIO_InitStruct.Pin = SOFTI2C2_SDA_PIN;
  HAL_GPIO_Init(SOFTI2C2_SDA_GPIO, &GPIO_InitStruct);

  SoftI2C2_delayTicks = Delay_GetSYSFreq() / speed / 2;
  SoftI2C2_halfTicks = (SoftI2C2_delayTicks >> 1) + 1;
}
...
uint8_t SoftI2C2_SendAddr(uint16_t addr, uint8_t addrLen, uint8_t RorW)
{
  ...
  if (addrLen == SI2C_ADDR_7b)
  {
    ...
    return SoftI2C2_SendByte_ACK(buf, SI2C_ACK);
  }
  else
  {
    ...
    buf |= 0x78;
    if (!SoftI2C2_SendByte_ACK(buf, SI2C_ACK))
      return 0;
...
uint8_t SoftI2C2_Read(uint16_t deviceAddr, uint8_t deviceAddrLen, uint8_t memAddr, uint8_t *dataBuf, uint32_t dataSize)
{
  ...
  SoftI2C2_Start();
  if (!SoftI2C2_SendAddr(deviceAddr, deviceAddrLen, SI2C_WRITE))
    return 0;
...
uint8_t SoftI2C2_Write(uint16_t deviceAddr, uint8_t deviceAddrLen, uint8_t memAddr, uint8_t *dataBuf, uint32_t dataSize)
{
  ...
  for (i = 0; i < dataSize; i++)
    if (!SoftI2C2_SendByte_ACK(*(dataBuf + i), SI2C_ACK))
      return 0;
  SoftI2C2_Stop();
...
void SoftI2C2_Start(void)
{
  ...
  SOFTI2C2_SDA(0);                  // START: when CLK is high,DATA change form HIGH to LOW
  Delay_ticks(SoftI2C2_delayTicks); // hold time

  SOFTI2C2_SCL(0); // cannot be read
...
{
  SOFTI2C2_SCL(0);
  Delay_ticks(SoftI2C2_halfTicks);
  SOFTI2C2_SDA(1);
  Delay_ticks(SoftI2C2_halfTicks);
  SoftI2C2_Start();
}
...
void SoftI2C2_SendACK(uint8_t ACK) // 0:ACK 1:NACK
{
  SOFTI2C2_SCL(0); // change start
  Delay_ticks(SoftI2C2_halfTicks);
  SOFTI2C2_SDA(ACK);
  Delay_ticks(SoftI2C2_halfTicks);  // data setup time
  SOFTI2C2_SCL(1);                  // can be read
  Delay_ticks(SoftI2C2_delayTicks); // hold
  SOFTI2C2_SCL(0);                  // cannot be read
  Delay_ticks(SoftI2C2_halfTicks);  // data setup time & SCL_LOW & SMBus requirement
}

uint8_t SoftI2C2_WaitACK(void) // 0:ACK 1:NACK/No response
{
  uint16_t waitTime = 0;
  uint8_t result = 0;
  SOFTI2C2_SDA_IN();
...
void SoftI2C2_SendByte(uint8_t byte) // barely send a byte
{
  int8_t i;
  SOFTI2C2_SCL(0);
  for (i = 7; i >= 0; i--)
  {
    SOFTI2C2_SDA((byte >> i) & 1u);
    Delay_ticks(SoftI2C2_halfTicks); // data setup time
    SOFTI2C2_SCL(1);
    Delay_ticks(SoftI2C2_delayTicks); // data hold time
    SOFTI2C2_SCL(0);
    Delay_ticks(SoftI2C2_halfTicks); // data setup time & SCL_LOW & SMBus requirement
  }
}
...
uint8_t SoftI2C2_ReadByte(void) // barely read a byte
{
  uint8_t i, result = 0;
  SOFTI2C2_SDA_IN();
  for (i = 0; i < 8; i++)
  {
    SOFTI2C2_SCL(0);
    Delay_ticks(SoftI2C2_delayTicks); // data setup time & SCL_LOW & SMBus requirement
    SOFTI2C2_SCL(1);
    result <<= 1;
    Delay_ticks(SoftI2C2_halfTicks); // data setup time
    result |= SOFTI2C2_READSDA();
    Delay_ticks(SoftI2C2_halfTicks); // data hold time
  }
  return result;
}
...
// If SI2C_ACK is given, it will retry SOFTI2C2_RETRYTIMES if no ACK is detected,
// and it will return whether the ACK is detected finally.
uint8_t SoftI2C2_SendByte_ACK(uint8_t byte, uint8_t handleACK) // handle ACK and retry
{
  uint8_t i;
  ...
  {
    for (i = 0; i < SOFTI2C2_RETRYTIMES; i++)
    {
      SoftI2C2_SendByte(byte);
      if (SoftI2C2_WaitACK() == SI2C_ACK)
        break;
    }
    return (i < SOFTI2C2_RETRYTIMES);
  }
}
...
{
  uint8_t result;
  result = SoftI2C2_ReadByte();
  SoftI2C2_SendACK(ACK);
  return result;
}
