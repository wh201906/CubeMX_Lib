...
void SoftI2C2_SendACK(uint8_t ACK) // 0:ACK 1:NACK
{
  ...
  SOFTI2C2_SDA_OUT();
  SOFTI2C2_SDA(ACK);
  Delay_ticks(SoftI2C2_delayTicks / 4*3); // data setup time
...
uint8_t SoftI2C2_ReadByte(void) // barely read a byte
{
  ...
  {
    SOFTI2C2_SCL(0);
    Delay_ticks(SoftI2C2_delayTicks / 2); // data setup time & SCL_LOW & SMBus requirement
    SOFTI2C2_SCL(1);
    result <<= 1;
    Delay_ticks(SoftI2C2_delayTicks / 2); // data setup time
    result |= SOFTI2C2_READSDA();
    Delay_ticks(SoftI2C2_delayTicks); // data hold time
  }
  return result;
